...
describe('publish confirm', () => {
  ...
  it('prints the plan and writes nothing without --yes', async () => {
    await packIt()
    // `ingest` matches a Studio export on this exact string, so a malformed id is a row no read can find.
    await expect(main(['publish', 'confirm', SLUG, '--video-id', 'abc123', '--at', '2026-09-18T15:00:00Z', '--yes', ...common()])).rejects.toThrow(/--video-id must be the 11-character YouTube id/)
    const { out, error } = await failing(['publish', 'confirm', SLUG, '--video-id', 'dQw4w9WgXcQ', '--at', '2026-09-18T15:00:00Z'])
    ...
    const plan = await json(['publish', 'confirm', SLUG, '--video-id', 'dQw4w9WgXcQ', '--at', '2026-09-18T15:00:00Z'], 0).catch((e: Error) => e)
    expect(plan).toBeInstanceOf(Error)
  })
  it('adds the ledger row with --yes, starting the review clock', async () => {
    await packIt()
    const j = await json(['publish', 'confirm', SLUG, '--video-id', 'dQw4w9WgXcQ', '--at', '2026-09-18T15:00:00Z', '--thumb-b', 'result-v2', '--yes'])
    expect(j.applied).toBe(true)
    const row = openStore(data).get('ledger', SLUG)
    expect(row).toMatchObject({ slug: SLUG, title: TITLE, videoId: 'dQw4w9WgXcQ', publishedAt: '2026-09-18T15:00:00.000Z', thumbA: 'stakes', thumbB: 'result-v2', updatedAt: new Date(NOW).toISOString() })
    ...
    const text = await run(['publish', 'confirm', SLUG, '--video-id', 'dQw4w9WgXcQ', '--at', '2026-09-18T15:00:00Z', '--yes'])
    expect(text.out).toMatch(/Recorded: scrap-solar published 2026-09-18T15:00:00.000Z as dQw4w9WgXcQ/)
    await expect(main(['publish', 'confirm', SLUG, '--video-id', 'dQw4w9WgXcQ', '--at', 'friday', '--yes', ...common()])).rejects.toThrow(/--at must be an ISO date/)
