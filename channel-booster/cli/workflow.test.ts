...
describe('booster workflow run', () => {
  ...
  it('runs a human stage against its evidence file: failed until the file exists, then passed', async () => {
    ...
    expect(failed.out).toMatch(/Stage plan FAILED \(by sam\): human step; evidence packages\/empty-sprinter-to-camper-in-90-days\/shots.md/)
    expect(failed.out).toMatch(/evidence file packages\/.*\/shots.md is missing/)
    expect(failed.out).toMatch(/Not advanced/)
    expect(failed.out).toMatch(/\[!\] plan {8}failed/)
    expect(openStore(data).get('workflows', SLUG)!.stages[3].status).toBe('failed')
    ...
    expect(code).toBe(0)
    expect(parsed.result).toMatchObject({ stageId: 'plan', kind: 'human', status: 'passed', evidence: 'shots.md', agent: 'sam' })
    expect(parsed.status.stages[3]).toMatchObject({ status: 'passed', agent: 'sam', gateResult: 'ok: shots.md exists' })
    expect(parsed.done).toBe(false)
    const status = await run(['workflow', 'status', SLUG, ...base()])
    expect(status.out).toMatch(/\[x\] plan {8}passed/)
    expect(status.out).toMatch(/Next: production/)
  })
  ...
  it('--stage runs a named stage, refuses out-of-order stages and dry-runs without writing', async () => {
    ...
    await override('packaging', 'b')
    await override('story', 'c')
    await expect(main(runFlags(['--stage', 'edit']))).rejects.toThrow(/stage "edit" is not runnable: "plan" is pending/)
    await expect(main(runFlags(['--stage', 'nope']))).rejects.toThrow(/has no stage "nope"/)
    const dry = await json(runFlags(['--stage', 'plan', '--dry-run']))
    expect(dry.parsed.result.status).toBe('dry-run')
    expect(openStore(data).get('workflows', SLUG)!.stages[3].status).toBe('pending')
    const dir = path.join(tmp, 'packages', SLUG)
    mkdirSync(dir, { recursive: true })
    writeFileSync(path.join(dir, 'shots.md'), '# shots\n')
    const real = await json(runFlags(['--stage', 'plan', '--agent', 'sam']))
    expect(real.code).toBe(0)
    expect(real.parsed.status.stages[3].status).toBe('passed')
  })
