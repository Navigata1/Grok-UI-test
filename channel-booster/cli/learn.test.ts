...
describe('booster rules', () => {
  it('compile turns the ledger into 00-learned-rules.md, show lists the store, and an accepted rule survives a compile', async () => {
    ...
    expect(content).toMatch(/^# Learned rules \(compiled\): hypotheses under observation/)
    expect(content).toContain('## Under observation: winning so far')
    expect(content).toContain(`- Hypothesis under observation: "${HYPOTHESIS}" may help on this channel (10 tests, `)
    ...
    expect(readFileSync(file, 'utf8')).toContain('## Accepted by a person\n\nThese are playbook rules: a person accepted them.\n- Never shout in the title [accepted by human]')
    const recompiled = await run(['rules', 'compile', '--half-life', '30', '--promote-tests', '5'])
    expect(recompiled.out).toContain(', 1 accepted by a person.')
    expect(recompiled.out).toContain('  accepted by human: Never shout in the title')
    ...
    expect(shown.out).toContain('  accepted by human: Never shout in the title')
