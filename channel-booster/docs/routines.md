...
| Monday, early | `npm run booster -- outliers <competitors.csv> --fresh --diff data/last-scan.json && npm run booster -- bank rescore && npm run booster -- brief --week` | Refreshes the outlier scan, wakes parked ideas whose topic got a fresh outlier, decays stale ones, and writes the Monday brief. |
...
0 6 * * 1    cd /path/to/repo && npm run booster -- bank rescore && npm run booster -- brief --week > channel-booster/data/brief.md
