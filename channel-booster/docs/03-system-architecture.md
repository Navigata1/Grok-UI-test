...
| 8 | Publish | agent assembles, human clicks | `publish pack`, `publish check`, then human uploads with Test & Compare on, `publish confirm --video-id --at [--levers ".." --predicted-ctr ..] --yes` | every checklist line true; ledger row with `publishedAt` and its pre-registered hypothesis exists | `packages/<slug>/publish.md`, ledger row |
