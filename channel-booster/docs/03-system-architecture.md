...
| 8 | Publish | agent assembles, human clicks | `publish pack`, `publish check`, then human uploads with Test & Compare on, `publish confirm --video-id --at [--levers ".." --predicted-ctr ..] --yes` | `publish.md` exists and `publish-check.json.pass == true` (all nine checklist lines); the ledger row with `publishedAt` and its pre-registered hypothesis is written separately by `publish confirm --yes` and is not part of this gate | `packages/<slug>/publish.md`, ledger row |
