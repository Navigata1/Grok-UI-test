...
Each component: what it is, the doctrine it serves, and the command, skill, prompt or file that implements it. Every component below is shipped; the file names in each heading say where it lives, and `booster help` lists the commands.
...
### 2.12 Store of record and sync (`src/store.ts`, `booster sync`)
