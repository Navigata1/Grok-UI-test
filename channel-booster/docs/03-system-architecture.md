...
Today `WorkflowStage.gate` is prose and `booster workflow` writes a static runbook. Each stage gains `run: {kind: 'command'|'human', command?: string[], artifact: string, evidence?: string}`, `gate: {kind: 'json-path-min'|'json-path-eq'|'file-exists'|'all-of', ...}`, `status`, `startedAt`, `finishedAt`, `gateResult`; the prose gate is kept as `gateText` so `workflow.test.ts` stays green. `evaluateGate(stage, cwd)` is pure and tested with fake artifacts; `runStage()` spawns `npm run booster -- ...` via `node:child_process`. `booster workflow run <slug> --next --agent <name>` executes the next runnable stage, evaluates its gate, writes status and a ledger event, and refuses to advance on failure; `--override --reason` is recorded and shown in the retro. Stages `demand`, `packaging`, `story`, `plan`, `thumbnail`, `publish`, `review48`, `postmortem` are commands; only `production` and `edit` are `human`, each with a required evidence file (`footage.txt`, `cut.txt`). The final pick and the publish click are human confirmations inside the `packaging` and `publish` stages, not stages of their own. `weeklyCadence({publishDay, perWeek, solo})` anchors the 48-hour review to a real weekday; `governCalendar()` warns (not refuses) above `maxPerWeek` and requires a written reason to proceed [H]. Serves R1, R2, R6.
...
Ten stages, which is what `generateWorkflow()` emits and what `workflow run <slug> --next` walks to the end. The 28-day learn — `rules compile` plus hypothesis scoring, writing `playbook/00-learned-rules.md` and `data/rules.jsonl` — is not an eleventh stage: it is the Sunday weekly job of section 4, because it learns across videos rather than driving one.
