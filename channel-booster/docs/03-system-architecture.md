...
One command produces a complete package and refuses to emit it until the deterministic gates pass: `scoreTitle >= 60` [H], `qaThumbnail` grade `ship` on at least two concepts with different `angle` enums, `titleThumbnailOverlap < 0.67` [H], and the package review verdict `pass`. The inlined review in `cli/booster.ts` (which today never calls `qaThumbnail`, so colour, emotion and background rules are skipped on the fast path) moves to `reviewPackage()` in `src/package.ts` and calls it. With a key, generation is `ai title-lab` + `ai thumbnail-factory`, and each failed gate's `fixes` are fed back as a "fix these before returning" turn for up to three rounds (`prompts/package-fix.md`); with `--offline`, the concepts come from `buildThumbnailBrief` and no title is generated: a formula with the topic pasted in reads "I Did A $300 solar generator Until It Worked", so the sheet lists `titleShapes()` (the formulas with a blank, no scores) and the title gate fails until a person passes `--title`. `scoreTitle` also holds template fills under the gate (mixed Title Case and lowercase, a pronoun or article pasted in after a frame word, "How I" plus a gerund). Output: `packages/<slug>/package.json` `{promise, titles[], chosenTitle, titleSource, titleShapes[], thumbnails[] with qa, abPick, designerBrief, hypothesis, rounds, gateReport}` plus `package.md`; `titleSource: 'person'` makes every rebuild, the workflow's packaging stage included, keep the person's title, with or without the model. Human writes three titles of their own on the sheet (the sheet has three blank lines) and picks the final title and pair. Command: `booster package build "<idea>"|<idea:id>|<slug> --promise ".." [--title ".."] [--rounds 3] [--offline]`, `booster package review --title ".." --thumb-text ".." [--elements ..]`. Serves R1, R2, R4.
