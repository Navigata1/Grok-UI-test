...
3. Build the package: `npm run booster -- package build "<idea>" --promise "<sentence>" [--offline] [--rounds 3]`. Read `packages/<slug>/package.md`. If any gate reads FAIL, the sheet says which fixes were fed back and what still failed; fix the promise or the inputs and rebuild before you gather anyone. Offline the title gate reads FAIL until step 8: the builder never picks a title, and the sheet lists the formula shapes (a blank, no scores) to write from.

## In the room (or alone, with a 12-hour gap before step 8)

4. Read the chosen title aloud (offline, read your best draft instead). Say what it tells. Cover it and look only at the A concept: say what it shows. If both sentences are the same, one of them is wasted; fix it now.
5. Write three titles of your own on the three blank lines of the sheet, by hand, in the channel's voice. Do not look at the generated list while you write. Score them: `npm run booster -- titles score "<title>"` (a formula with the topic pasted in scores under the gate on purpose). Keep any that beats the chosen one.
...
8. Pick the final title and the final A/B pair. Solo: do this the next morning, not now; the 12-hour gap is the second reviewer. Put the title in: `npm run booster -- package build <slug> --title "<final title>" --lever "<what the title tests>"`. Every rebuild keeps both, the workflow's packaging stage included; the lever is pre-registered with the A/B angles, so `rules compile` can learn from the title too.
