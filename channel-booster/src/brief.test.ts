...
describe('buildBrief', () => {
  ...
  it('lists decisions that need approval or application, and skips HOLD, WAIT and applied ones', () => {
    ...
    expect(byId['a:48'].command).toBe('booster repackage prepare a then booster decide approve a --bucket 48 --by <name> --yes')
    expect(byId['a:48'].flipCondition).toBe('Flips to HOLD if the window closes.')
    expect(byId['b:48'].text).toContain('approved by jony')
    expect(byId['b:48'].command).toBe('start Test & Compare in Studio, then booster decide apply b --bucket 48 --by <name> --yes')
    expect(byId['c:168'].command).toBe(`booster bank approve ${sequel.id} --yes`)
    expect(byId['d:168'].command).toMatch(/^booster bank add/)
    expect(byId['e:168'].command).toBe('booster bank park <idea-id> --reason "no audience at 168 h" then booster decide approve e --bucket 168 --by <name> --yes')
  })
  ...
  it('lists open experiments, ready once past the hour floor (the cold-start floor without a baseline)', () => {
    ...
    expect(established.experimentsToClose[0].command).toBe('type the Test & Compare panel, then booster test judge --slug a --a "<impressions>,<ctr>,<share>" --b "<impressions>,<ctr>,<share>" --hours 100 --record')
    expect(established.experimentsToClose[1].command).toBe('booster brief --today')
  ...
  it('lists rules that moved inside the window with their evidence counts, compiled ones as observations', () => {
    const base = { slugs: [], source: 'cli' }
    store.upsert('rules', RuleDoc.parse({ ...base, id: 'rule:1', rule: 'Prefer "numbers" on this channel', lever: 'numbers', tests: 4, wins: 3, confidence: 0.7, status: 'promoted', updatedAt: iso(2) }))
    ...
    store.upsert('rules', RuleDoc.parse({ ...base, id: 'rule:5', rule: 'Face on every how-to', status: 'promoted', acceptedBy: 'jony', confidence: 0.4, updatedAt: iso(3) }))
    const week = buildBrief(store, { now, profile: profileWith() })
    expect(week.rulesChanged.map((r) => r.id)).toEqual(['rule:1', 'rule:3', 'rule:5', 'rule:2'])
    // A compiled rule is a hypothesis under observation, even when an older compile stored instruction wording.
    expect(week.rulesChanged[0].text).toBe('under observation, winning so far: "numbers" (4 tests, 3 wins, smoothed win rate 67%, confidence 70%)')
    expect(week.rulesChanged[0].rule).toBe('Hypothesis under observation: "numbers" may help on this channel')
    expect(week.rulesChanged[0].command).toBe('test it again in a new package; only a person makes it a playbook rule: booster retro --accept-rule "<rule in your own words>" --into playbook/<file>.md --yes')
    expect(week.rulesChanged[1].text).toBe('under test: "stakes over results" (1 test, 1 win, smoothed win rate 67%, confidence 50%)')
    expect(week.rulesChanged[1].command).toBe('leave it to earn tests; booster rules compile after the next 7-day read')
    expect(week.rulesChanged[2].text).toBe('accepted by jony: Face on every how-to')
    expect(week.rulesChanged[2].command).toBe('booster rules compile (keeps it as written in playbook/00-learned-rules.md)')
    expect(week.rulesChanged[3].text).toBe('under observation, losing so far: "faces on every thumbnail" (3 tests, 1 win, smoothed win rate 40%, confidence 30%)')
    expect(week.rulesChanged[3].command).toBe('test it again on purpose or drop the lever; booster rules compile refreshes playbook/00-learned-rules.md')
    const md = renderBriefMarkdown(week)
    expect(md).not.toMatch(/prefer|is promoted|is retired/i)
  ...
  it('lists idea movers inside the window and the next three ideas with their commands', () => {
    ...
    expect(brief.ideaMovers[0].command).toBe(`booster bank approve ${fresh.id} --yes (or booster bank park ${fresh.id} --reason "<weakest axis>")`)
    ...
    expect(brief.nextThree[2].command).toBe(`booster bank approve ${fresh.id} --yes`)
  ...
  it('prints commands the CLI accepts: every human-only gate carries --yes and nothing names profile init', () => {
    seedHistory(3)
    addRow(store, { slug: 'due', title: 'Due', publishedAt: iso(2), now })
    addRow(store, { slug: 'extra', title: 'Extra', publishedAt: iso(1), now })
    const base = { numbers: {}, updatedAt: iso(1), source: 'cli' }
    store.upsert('decisions', DecisionDoc.parse({ ...base, id: 'a:48', slug: 'a', bucket: '48', decision: 'REPACKAGE' }))
    store.upsert('decisions', DecisionDoc.parse({ ...base, id: 'b:48', slug: 'b', bucket: '48', decision: 'RE-TEST-TITLE', approvedBy: 'jony', approvedAt: iso(0.5) }))
    store.upsert('decisions', DecisionDoc.parse({ ...base, id: 'c:168', slug: 'c', bucket: '168', decision: 'SEQUEL' }))
    store.upsert('decisions', DecisionDoc.parse({ ...base, id: 'e:168', slug: 'e', bucket: '168', decision: 'PARK' }))
    addIdea(store, { idea: 'Sequel: the c video', scores, sequelOf: 'c', now })
    store.upsert('experiments', ExperimentDoc.parse({ id: 'x:1', slug: 'x', variants: [{ name: 'A' }, { name: 'B' }], startedAt: iso(20), updatedAt: iso(1), source: 'cli' }))
    store.upsert('rules', RuleDoc.parse({ id: 'rule:1', rule: 'stakes over results', status: 'candidate', updatedAt: iso(1), source: 'cli' }))
    addIdea(store, { idea: 'Fresh', scores, now })
    const brief = buildBrief(store, { now, profile: profileWith({ baselines: baselines(), maxPerWeek: 1 }), inboxFiles: [] })
    const commands = everyItem(brief).map((i) => i.command)
    expect(commands.length).toBeGreaterThanOrEqual(8)
    // `profile init` refuses to run once channel.json exists, and --force rewrites it from the init answers alone.
    expect(brief.alerts.map((a) => a.kind)).toContain('cadence-over-cap')
    expect(brief.alerts.find((a) => a.kind === 'cadence-over-cap')?.command).toBe('hold the next publish a week, or raise the cap on purpose: edit maxPerWeek in channel.json by hand (raising the cap is a human-only gate, and only with the trailing four rows at 0.9x baseline CTR)')
    expect(commands.filter((c) => c.includes('booster profile init'))).toEqual([])
    // Every gate AGENTS.md hands to a person: the printed line carries the --yes the command asks for.
    const gates = [/booster bank approve \S/, /booster decide approve \S/, /booster decide apply \S/, /booster set \S+ --bucket 168 /, /booster retro --accept-rule /]
    for (const c of commands) for (const gate of gates) if (gate.test(c)) expect(c).toContain('--yes')
    // `decide apply` also refuses without --by.
    for (const c of commands.filter((c) => c.includes('booster decide apply '))) expect(c).toContain('--by ')
    // `test judge` takes the slug as --slug and both arms, and `brief` takes no positional words.
    for (const c of commands.filter((c) => c.includes('booster test judge '))) expect(c).toMatch(/booster test judge --slug \S+ --a "[^"]+" --b "[^"]+" --hours \d+/)
    for (const c of commands.filter((c) => c.includes('booster brief'))) expect(c).toMatch(/booster brief( --today| --week)?$/)
  })
...
describe('renderBriefMarkdown', () => {
  it('renders one section per list with the command on every line and a named empty state', () => {
    ...
    expect(empty).toContain('## Next three\n\n- no idea is banked or green (ideas in packaging or production do not count): booster bank add "<idea>" --score "..."')
    ...
    expect(md).toContain(`## Idea movers (1)\n\n- "Cold showers for 30 days" new in the bank · 85/100 green — do: \`booster bank approve ${idea.id} --yes`)
  ...
  it('does not call the bank empty when every idea has moved on to packaging', () => {
    const idea = addIdea(store, { idea: 'I Tested Every Portable Power Station', scores, now: daysAgo(3) })
    setStatus(store, idea.id, 'green', { now: daysAgo(2) })
    setStatus(store, idea.id, 'packaging', { now: daysAgo(1) })
    const brief = buildBrief(store, { now, profile: profileWith() })
    expect(brief.nextThree).toEqual([])
