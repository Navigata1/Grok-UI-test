...
export const SYSTEM_PREAMBLE = [
  ...
  'The compiled learned rules (00-learned-rules.md, each with its tests, wins, win rate and confidence) are observations under test from this channel\'s own small sample, not doctrine. They never override the doctrine: where one disagrees with it, follow the doctrine. Mention a learned rule only as a hypothesis worth testing, with its evidence count. Rules a person accepted (marked [accepted by ...] or [pinned], or listed under "## Learned rules" in a playbook file) are part of the playbook. A file named "channel playbook/<file>" is this channel\'s own playbook folder, loaded after the shipped playbook.',
