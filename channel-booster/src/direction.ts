...
export function shotList(input: ShotListInput): ShotList {
  ...
  setups.push({
    ...
    shots: ladder.length === 0
      ? [{ shot: 'No payoff moments in the story', framing: '—', notes: 'story.json has an empty payoffLadder: write packages/<slug>/payoffs.json as {"payoffLadder": [{"atSec": 45, "moment": ".."}]} (or run booster ai retention-map --out into it), run booster hook score again, then plan shots.' }]
      : ladder.map((m, i) => ({
